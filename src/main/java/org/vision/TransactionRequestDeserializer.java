...
public class TransactionRequestDeserializer extends StdDeserializer<TransactionRequest> {
    ...
    // Converts "YYYY-MM-DDTHH:mm:ssZ" to Unix epoch seconds using integer arithmetic only.
    private static long isoToEpoch(String iso) {
        int y = parseInt4(iso, 0), m = c2(iso, 5), d = c2(iso, 8);
        int h = c2(iso, 11), mi = c2(iso, 14), s = c2(iso, 17);
        if (m <= 2) { y--; m += 12; }
        long jd = 365L * y + y / 4 - y / 100 + y / 400 + (153 * m - 457) / 5 + d - 719469;
        return jd * 86400 + h * 3600 + mi * 60 + s;
    }
    ...
    public TransactionRequest deserialize(JsonParser p, DeserializationContext ctx) throws IOException {
        ...
        while (p.nextToken() != JsonToken.END_OBJECT) {
            ...
            switch (field) {
                ...
                case "last_transaction" -> lastTransaction = parseLastTransaction(p, transaction != null ? transaction.requestedAtEpoch() : 0L);
    ...
    private TransactionRequest.Transaction parseTransaction(JsonParser p) throws IOException {
            ...
            switch (field) {
                ...
                case "requested_at" -> {
                    ...
                    if (iso != null && iso.length() >= 19) {
                        ...
                        requestedAtEpoch = isoToEpoch(iso);
                    }
                }
                default -> p.skipChildren();
            }
        ...
        return new TransactionRequest.Transaction(amount, installments, hour, dayOfWeek, requestedAtEpoch);
    }
    ...
    private TransactionRequest.LastTransaction parseLastTransaction(JsonParser p, long txEpoch) throws IOException {
        ...
        while (p.nextToken() != JsonToken.END_OBJECT) {
            ...
            switch (field) {
                case "timestamp" -> {
                    String ts = p.getText();
                    if (ts != null && ts.length() >= 19 && txEpoch > 0) {
                        secondsAgo = (int) Math.max(0L, txEpoch - isoToEpoch(ts));
                    }
                }
        ...
        return new TransactionRequest.LastTransaction(secondsAgo, kmFromCurrent);
    }
}
